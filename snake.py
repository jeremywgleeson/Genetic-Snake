...
class Snake:
    ...
    def addFood(self):
        if len(self.spacesTaken) == BOARD_SIZE**2:
            print("You win!")
        self.foodPos = [random.randint(0, BOARD_SIZE), random.randint(0, BOARD_SIZE)]
    ...
    def goInDirection(self, direction):
        if -direction[0] == self.lastDirection[0] or direction[0] == self.lastDirection[0]:
            ...
            if nextMove[0] >= BOARD_SIZE or nextMove[1] > BOARD_SIZE or nextMove[0] <= 0 or nextMove[0] <= 0:
                self.die()
                return 1
    ...
    for row in range(BOARD_SIZE):
        for col in range(BOARD_SIZE):
            if [col,row] in s.spacesTaken:
                pygame.draw.rect(screen, GREEN, [(1 + SQUARE_SIZE) * col + 1, (1 + SQUARE_SIZE) * row + 1, SQUARE_SIZE, SQUARE_SIZE])
            else:
                pygame.draw.rect(screen, BLACK, [(1 + SQUARE_SIZE) * col + 1, (1 + SQUARE_SIZE) * row + 1, SQUARE_SIZE, SQUARE_SIZE])
            if [col, row] == s.foodPos:
                pygame.draw.circle(screen, RED, [int((1 + SQUARE_SIZE) * col + 1), int((1 + SQUARE_SIZE) * row + 1)], int(SQUARE_SIZE // 2))
    time.sleep(0.5)
    pygame.display.flip()
